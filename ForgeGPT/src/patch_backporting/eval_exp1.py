...
def run_eval_exp1(max_workers: int = None):
    ...
    output_path = vim_neovim_file_path.replace(".json", "_exp1_result.json")
    output_raw_path = vim_neovim_file_path.replace(".json", "_exp1_raw_result.json")
    ...
    def worker(i, item):
        ...
        pre_transform_code = item.get('func_before_source')
        post_transform_code = item.get('func_before_target')
        after_src_code = item.get('func_after_source')
        post_transform_after_src_code = item.get('func_after_target')

        instruction = "Generate DSL code that transforms the original program to the target program."
        try:
            res = main_impl(
                repo=None,
                ...
                target_code=after_src_code,
                expected_post_transform_target_code=post_transform_after_src_code,
                dry_run=True,
