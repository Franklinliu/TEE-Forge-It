...
def get_upstream_branch(repo_path: str) -> str:
    ...
    try:
        ...
        if "_sgx" in current_branch:
            current_branch = current_branch.split("_sgx")[0]
        # Assume the upstream branch has the same name as the current branch
        
        subprocess.check_output(
                ["git", "show-ref", "--verify", "--quiet", f"refs/remotes/upstream/{current_branch}"],
                cwd=repo_path,
                text=True
        )
        ...
        # we need to check if upstream/master exists
        try:
            subprocess.check_output(
            ...
            # otherwise use upstream/main
            # If error occurs, fallback to upstream/main
            return f"upstream/main"  # Fallback to upstream/main if error occurs
...
def get_changed_files_since_fork(repo_path: str, fork_point: str) -> List[str]:
    ...
    try:
        ...
        rust_files = [file for file in changed_files if file.endswith(".rs") or file.endswith(".toml")]
        return rust_files
    except subprocess.CalledProcessError as e:
